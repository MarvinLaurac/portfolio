  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center items-center gap-2 pt-5 px-4 pointer-events-none">
      ...
      <div className="flex md:hidden pointer-events-auto w-full flex-col gap-2">
        ...
        {menuOpen && (
          <nav className={`flex flex-col w-full px-2 py-2 gap-1 rounded-2xl transition-all duration-300 ${pillClass}`}>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                onClick={() => setMenuOpen(false)}
                className="w-full text-left px-4 py-3 rounded-xl text-[14px] font-medium tracking-wide text-white/50 hover:text-white/90 hover:bg-white/5 transition-all duration-200"
              >
                {link.label}
              </Link>
            ))}
